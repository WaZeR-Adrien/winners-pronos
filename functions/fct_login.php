...
function addUser($nom,$prenom,$email,$login,$pw){
    $query = 'INSERT INTO user(nom,prenom,email,login,password,role_id) VALUES ("'.$nom.'","'.$prenom.'","'.$email.'","'.$login.'","'.sha1($pw).'", 2)';
...
function connexion($login,$pw){
    ...
    if ($res == true){
        ...
        redirect($_SERVER['HTTP_REFERER'].'?co=success');
    } else {
        redirect($_SERVER['HTTP_REFERER'].'?co=error');
    }
}